...
ChkUtil.isNull = function(str) {
	var newstr = str.replace(/(^\s*)|(\s*$)/g, "");// 删除二边空格
	if (null == newstr || "" == newstr) {
		return true;
	} else {
		return false;
	}
...
ChkUtil.pwdLevel = function(value) {
	...
	if (pattern_1.test(value)) {
		...
		var level31 = pattern_7.test(value)
				&& ((pattern_3.test(value) && pattern_5.test(value) && pattern_6
						.test(value)) || (pattern_4.test(value)
						&& pattern_5.test(value) && pattern_6.test(value)));
		var level32 = pattern_2.test(value);
		var level33 = pattern_3.test(value) && pattern_4.test(value)
				&& pattern_5.test(value) && pattern_6.test(value);
...
/**
 * 随便写了个更加精准的倒计时方法,不会因为切到后台或前进后退而停顿 ChkUtil.countDown
 * 
 * @param (number)initialTime
 *            计时的初始时间 (boolean)recycleFlag 是否重置初始时间
 * @return var time = { (number)nowTime : 当前时间毫秒数, (number)interval :
 *         距离第一次调用本方法已经过了多久毫秒数 (number)countDown60s : 倒计时60s所剩余的描述
 *         (number)countDown10Mins_m : 倒计时10分钟剩余分钟数 (number)countDown10Mins_s :
 *         倒计时10分钟剩余的秒数部分 };
 * @throws
 * @author zhuzheng ()-().----. .
 * @date 2015年10月10日
 * @tips 调用时假如需要传参数，请使用匿名函数来传
 *       setTimeout(function(){ChkUtil.countDown(time,true);},1000);
 * @param type
 *            用来记录，是那个地方要使用的，存cookie以保存, \"/` ___ ;________.'
...
ChkUtil.countDown = function(type, checkNewFlag) {
	 ...
	 * ChkUtil.initialTime = ChkUtil.getCookie(type); }else if(checkNewFlag ==
	 * 'Y'){//上次倒计时结束了，开始新得倒计时 ChkUtil.saveCookie3(type,now,1);//一分钟有效期的cookie
	...
	countDownTime.countDown60s = ((60 - interval / 1000) > 0 ? (60 - interval / 1000)
			: 0).toFixed(0);
	countDownTime.interval = interval;
	countDownTime.jsNowTIme = jsNowTIme;
	countDownTime.initialTime = ChkUtil.initialTime;
	// countDown10Mins_m : 9 - ChkUtil.countDownTime.interval/60000>0?9 -
	// ChkUtil.countDownTime.interval/60000:0,
