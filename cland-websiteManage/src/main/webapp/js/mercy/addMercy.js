	...
	editor = K.create('#sNewContent',
			{
				items : [ 'source', '|', 'fullscreen', 'undo', 'redo', 'print',
						...
						'advtable', 'hr', 'emoticons', 'link', 'unlink', 'lineheight' ],
...
$(document).ready(function() {
	...
	$('#sMediaImageFile').fileuploader({
        ...
        addMore: false,
        files: handleInit($('#sMediaImage').val()),
        clipboardPaste: 2000,
        listInput: true,
        enableApi: true,
		listeners: {
...
function doSummit() {
	...
	if (ChkUtil.isNull(sTitle)) {
		...
		BootstrapDialog.alert("请输入发布时间！");
		return false;
	} else if (ChkUtil.isNull(sMediaImage)) {
		BootstrapDialog.alert("请上传列表图片！");
		return false;
	} else if (ChkUtil.isNull(sIntroduce)) {
		BootstrapDialog.alert("请输入慈善介绍！");
		return false;
	} else if (ChkUtil.getLength(sIntroduce) > 200) {
		BootstrapDialog.alert("输入的慈善介绍过长！");
		return false;
