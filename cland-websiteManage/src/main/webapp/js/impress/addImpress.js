        ...
        onSelect: function(item, listEl, parentEl, newInputEl, inputEl) {
            ...
            reader.onload = function (e) {
                ...
                image.onload=function(){
                    ...
                    if(width > 268 || height > 362) {
                    	BootstrapDialog.alert("上传图片宽度不得超过268像素,高度像素不得超过362px");
                    	$('#news .fileuploader-action-remove').trigger('click');
                    	return false;
                    }else {
                    	return true;
                    }
...
function doSummit() {
	...
	var sBannerImage = $('#sBannerImage').val();
	var sLinkUrl = $('#sLinkUrl').val();
	if (ChkUtil.isNull(sTitle)) {
		alert("请输入期数");
		return false;
	} else if (ChkUtil.isNull(sBannerImage)) {
		alert("请上传封面图片！");
		return false;
	} else if (ChkUtil.isNull(sLinkUrl)) {
		alert("请输入链接！");
		return false;
	} else {
		$('#news').submit();
