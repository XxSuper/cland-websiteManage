...
function doSummit() {
	editor.sync();
	var sFatherNo = $('#sFatherNo').val();
	var sTitle = $('#sTitle').val();
	var sProfile = $('#sProfile').val();
	var iSortNum = $('#iSortNum').val();
	if (ChkUtil.isNull(sTitle)) {
		BootstrapDialog.alert("请输入标题！");
		return false;
	} else if (ChkUtil.isNull(sProfile)) {
		BootstrapDialog.alert("请输入栏目介绍！");
		return false;
	} else if (ChkUtil.getLength(sProfile) > 200) {
		BootstrapDialog.alert("输入的栏目介绍过长！");
		return false;
	} else if (ChkUtil.isInteger(iSortNum)) {
		BootstrapDialog.alert("输入的排序非整数！");
		return false;
	} else {
		$('#column').submit();
	}
