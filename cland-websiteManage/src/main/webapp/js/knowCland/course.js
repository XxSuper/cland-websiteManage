...
function showEdit(newsNo) {
    window.location.href = ctx + "/knowCland/showEditCourse.htm?newsNo=" + newsNo;
}
 ...
 */
function showSave() {
    window.location.href = ctx + "/knowCland/showSaveCourse.htm";
}

function doSummitCourse() {
    var dPublishDate = $('#dPublishDate').val();
    var sIntroduce = $('#sIntroduce').val();
    // editor.sync();
    if (ChkUtil.isNull(dPublishDate)) {
        alert("时间不能为空！");
        return false;
