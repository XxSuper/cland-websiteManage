...
<script src="${ctx}/js/mercy/mercyManage.js?20170103"></script>
	...
	<ul class="breadcrumb">
		...
		<li><a href="javascript:void(0)">公益慈善管理</a></li>
	</ul>
	
	<div >
		<!-- search button -->
		<form:form id="searchForm" modelAttribute="page" action="${ctx}/mercy/mercyQuery.htm" method="post" class="form-inline">
				...
				<button type="button" class="btn blue" onclick="showSaveMercy()"><i class="icon-plus icon-white"></i> 添加 </button>
		...
		<table id="contentTable" class="table table-striped table-bordered table-hover">
			...
			<thead>
				<tr>
					...
					<th style='width:10%'>首页图片</th>
					<th style='width:10%'>列表页图片</th>
				...
				<c:forEach items="${page.list}" var="newinfo" varStatus="varStatus">
					<tr>
						...
						<td>
							...
							<c:if test="${ empty newinfo.sBannerImage}">
							</c:if>
						</td>
						<td><img src="${newinfo.sMediaImage}" style='height:30px;' /></td>
						...
						<td>
							...
							<c:choose>
								<c:when test="${newinfo.iHomeDisplay == 1}">
									<a href='javascript:;' onClick="setHomeDisplay('${newinfo.sNewsNo}','1')">
										<i class="icon-edit icon-white"></i>取消首页展示
									</a>
								</c:when>
								<c:otherwise>
									<a href='javascript:;' onClick="setHomeDisplay('${newinfo.sNewsNo}','0')">
