...
<script src="${ctx}/js/cultureidea/ideaManage.js?20170103"></script>
	...
	<ul class="breadcrumb">
		...
		<li><a href="javascript:void(0)">文化理念管理</a></li>
	</ul>
	
	<div >
		<!-- search button -->
		<form:form id="searchForm" modelAttribute="page" action="${ctx}/culture/cultureIdeaQuery.htm" method="post" class="form-inline">
				...
				<button type="button" class="btn blue" onclick="showSaveIdea()"><i class="icon-plus icon-white"></i> 添加 </button>
				
				<input id="show" name="show" type="hidden" value="1" />

				<input id="iType" name="iType" type="hidden" value="9" />
		...
		<table id="contentTable" class="table table-striped table-bordered table-hover">
			...
			<thead>
				<tr>
					<th style='width:5%'></th>
					<th style='width:28%'>图片一</th>
					<th style='width:28%'>图片二</th>
			...
			<tbody>
				<c:forEach items="${page.list}" var="newinfo" varStatus="varStatus">
					<tr>
						...
						<c:forEach var="mediaImage" items="${fn:split(newinfo.sMediaImage,'|')}">
		                    <td><img src="${mediaImage}" style='height:30px;' /></td>
		                </c:forEach>
						<td>
							<a href='javascript:;' onClick="showEditIdea('${newinfo.sNewsNo}')" id="${newinfo.sNewsNo}">
