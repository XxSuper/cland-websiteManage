...
<script src="${ctx}/js/brand/brandManage.js"></script>
	...
	<ul class="breadcrumb">
		...
		<li><a href="javascript:void(0)">旗下品牌管理</a></li>
	</ul>
	
	<div >
		<!-- search button -->
		<form:form id="searchForm" modelAttribute="page" action="${ctx}/brand/brandQuery.htm" method="post" class="form-inline">
				...
				<button type="button" class="btn blue" onclick="showSaveBrands()"><i class="icon-plus icon-white"></i> 添加 </button>
		...
		<table id="contentTable" class="table table-striped table-bordered table-hover">
			...
			<thead>
				<tr>
					...
					<th style='width:10%'>logo的hover状态</th>
			...
			<tbody>
				<c:forEach items="${page.list}" var="brand" varStatus="varStatus">
					<tr>
						<td>
							<input type="checkbox" value="${brand.sNewsNo}" name="brandId">
						</td>
						<td>${brand.iSortNum}</td>
	                	<c:forEach var="img" items="${fn:split(brand.sMediaImage,'|')}">
		                    <td><img src="${img}" style='height:30px;' /></td>
		                </c:forEach>
						<td>${brand.sNewContent}</td>
						<td>
							<a href='javascript:;' onClick="editBrand('${brand.sNewsNo}')" id="${brand.sNewsNo}">
								<i class="icon-edit icon-white"></i>编辑
							</a>
							<a href='javascript:;' onClick="deleteBrand('${brand.sNewsNo}')" id="${brand.sNewsNo}">
								<i class="icon-trash icon-white"></i>删除
							</a>
						</td>
						<td>
							<input class='span8 js_px_input' type='text' id="${brand.sNewsNo}" value='${brand.iSortNum}' />
						</td>
					</tr>
				</c:forEach>
			</tbody>
