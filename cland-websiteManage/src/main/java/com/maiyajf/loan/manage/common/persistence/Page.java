...
import com.maiyajf.loan.manage.common.utils.CookieUtils;
...
public class Page<T> {
  ...
  private int toDoAudit = 0;// 待审核数
  private int auditPass = 0;// 审核通过数
  private int auditRefuse = 0;// 拒绝数
  private int auditBlack = 0;// 黑名单 数
  private int auditTotal = 0;// 总数
  ...
  public int getToDoAudit() {
    return toDoAudit;
  }

  public void setToDoAudit(int toDoAudit) {
    this.toDoAudit = toDoAudit;
  }
  ...
  public int getAuditRefuse() {
    return auditRefuse;
  }

  public void setAuditRefuse(int auditRefuse) {
    this.auditRefuse = auditRefuse;
  }



  public int getAuditBlack() {
    return auditBlack;
  }

  public void setAuditBlack(int auditBlack) {
    this.auditBlack = auditBlack;
  }
  ...
  public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize) {
    ...
    if (StringUtils.isNumeric(no) && !no.equals("")) {
   ...
   * @return List<T>
   */
  public List<T> getPageListByPageNo(List<T> result) {
      ...
      if (pageNo > 1) {
        ...
        if ((pageNo - 1) * pageSize > size) {
          ...
          for (int i = 0; i < pageNo; i++) {
            j = i;
            if ((pageNo - 1 - i) * pageSize < size) {
              break;
            }
          }
          return result.subList((pageNo - 1 - j) * pageSize,
              ((pageNo - j) * pageSize > size ? size : (pageNo - j) * pageSize));
        } else {
          return result.subList((pageNo - 1) * pageSize,
              (pageNo * pageSize > size ? size : pageNo * pageSize));
        }
      } else {
